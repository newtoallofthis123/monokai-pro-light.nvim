...
M.get = function(filter)
  local filters = { "classic", "machine", "octagon", "pro", "ristretto", "spectrum", "light"}

  if not vim.tbl_contains(filters, filter) then
    local msg = 'Invalid filter, expected "classic", "machine", "octagon", "pro", "ristretto", "light" or "spectrum"'
    local level = "info"
    filter = "pro"
    Util.log(msg, level)
  end
